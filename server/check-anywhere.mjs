...
/* ---------- 十九轮：动态地点跨境判定 / 备选按实际结果生成 ---------- */
{
  ...
  /* 二十轮 P1-2：备选不声称存在未展示的其它枢纽 */
  ok(one.fragile_legs.every((fl) => !fl.fallbacks.some((fb) => fb.includes('其它枢纽方向'))),
    'P1-2（二十轮）：无其它枢纽卡时备选不含「其它枢纽方向」句');
  ok(one.fragile_legs[0].fallbacks.length === 2, 'P1-2（二十轮）：悉尼备选仅直达假设+升级句两条');
  ...
  ok(m3.fragile_legs.some((fl) => fl.fallbacks.some((fb) => fb.includes('泛化方向参考'))) &&
    m3.fragile_legs.some((fl) => fl.fallbacks.some((fb) => fb.includes('同结果页还有同枢纽'))),
    'P1-2 回归：阿拉木图备选含泛化探索提示与同枢纽组合（不称其它枢纽方向）');
  const dg = r.candidates.find((c) => c.kind === 'direct');
...
/* ---------- planAnywhere 编排（注入桩，无网络无 key） ---------- */
{
  ...
  ok(r6.planner_version === 'v0.42.4', '编排：anywhere 版本号对齐 v0.42.4');
