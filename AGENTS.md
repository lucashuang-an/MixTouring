...
- **当前阶段（2026-09-25）**：旧产品 Phase 1–3、v1.2 的 G1/G2、G2.5、G2.6 已交付；G3 的取舍、脆弱段备选、探索详情、可选停留与修改已实现，移动／桌面可访问预览的全路径目测验收待补。**指定日期可执行往返比较、经证据验证的城市停留完整旅程与社区试点尚未实现**。北京 ⇄ 阿拉木图、2026 国庆附近为第一验证 case；外部证据缺口仍按探索／部分核验处理。
- 开发阶段路线与交接细节见 §6。
...
**当前：v1.2 G3 探索流程实现阶段**——取舍、脆弱段备选、详情、停留与修改已有代码和回归；可访问预览环境中的移动／桌面完整路径目测尚未完成。G0 的北京—阿拉木图指定日期证据取样继续独立进行，不阻挡探索体验；指定日期可执行国际往返和社区试点尚未交付。
