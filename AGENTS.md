...
- **当前阶段（2026-09-25）**：旧产品 Phase 1–3、v1.2 的 G1/G2、G2.5、G2.6 已交付；G3 探索版的取舍、脆弱段备选、详情、可选停留与修改已完成移动／桌面全路径验收，可进入 G4 私人保存与用户测试。**指定日期可执行往返比较、经证据验证的城市停留完整旅程与社区试点尚未实现**。北京 ⇄ 阿拉木图、2026 国庆附近为第一验证 case；外部证据缺口仍按探索／部分核验处理。
- 开发阶段路线与交接细节见 §6。
...
**当前：v1.2 G3 探索流程验收完成，进入 G4 前置阶段**——取舍、脆弱段备选、详情、停留与修改已有代码、回归和移动／桌面实际路径验收。G0 的北京—阿拉木图指定日期证据取样继续独立进行，不阻挡探索体验；指定日期可执行国际往返和社区试点尚未交付。
