...
- **产品方向（2026-09-23 调整）**：长期面向独立旅行者，帮助其发现原本没想到的旅行选择、理解取舍与可信边界；首批从 Solo Trip 用户验证，第一环节先交付「值得探索的走法」与推荐理由，城市停留完整旅程随后建设。
- **核心价值**：依据地理顺路性、交通枢纽与已知连接提出有道理的探索候选，说明相对直达的取舍及待查项；不要求每条探索候选先做指定日期人工核验。直达占优时如实说明；只有逐段证据齐备才给可执行与确定省钱结论。
- **定位**：纯内容与方案产品，**不做买票/交易服务**。
- **首批目标用户**：已有目的地、希望独自安排旅程的 Solo Trip 旅行者；独处、交流、同行分别由用户自愿选择。Web 先行（桌面侧边栏 + 移动端底栏响应式）。
- **当前阶段（2026-09-25）**：旧产品 Phase 1–3、v1.2 的 G1/G2 已交付；G2.5 统一入口与探索骨架、G2.6 线路发现首卡已收口（PR #10/#11 十四/十七轮评审通过合并）。下一阶段直接进入 G3：先理清页面职责，把原拟 G2.7 的相对直达取舍与脆弱段备选方向纳入 G3 首段，再做详情、停留和修改；G3 结束后统一走查移动／桌面完整用户路径。**指定日期可执行往返比较、城市停留完整旅程与社区试点尚未实现**。北京 ⇄ 阿拉木图、2026 国庆附近为第一验证 case；外部证据缺口仍按探索／部分核验处理。
- 开发阶段路线与交接细节见 §6。
...
- **探索建议可先交付**：地理与交通区位可支持「值得探索」的路线假设和比较理由，不等于某段存在指定日期班次；候选要有可解释的连接依据并经抽样校准，不承诺「80% 准确率」。
...
**当前：v1.2 G3 开发准备阶段**——G2.6 线路发现首卡已收口；G0 的北京—阿拉木图指定日期证据取样继续独立进行，不阻挡探索体验。G3 纳入取舍说明、脆弱段备选、详情、停留、修改及完成后的全路径整合验收；具体门槛见 `开发流程_SoloTrip_v1.2.md`。指定日期可执行国际往返和社区试点尚未交付。
